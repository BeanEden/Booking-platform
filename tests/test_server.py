...
def test_showSummary(client):
    ...
    data = rv.data.decode()
    assert rv.status_code == 200
    assert data.find('<a href="/book/Spring%20Festival/'
                     'Iron%20Temple">Book Places</a>') != -1


def test_purchasePlace_booking_should_work(client, club_one,
                                           competition_with_club_one):
    ...
    rv = client.post('/purchasePlaces',
                     data=dict(club=club_one["name"],
                               competition=compet_loaded["name"],
...
def test_loadPlacesAlreadyBooked_no_club_should_return_zero(
        competition_without_club_list, club_one):
    competition = competition_without_club_list
    club = club_one
    assert loadPlacesAlreadyBooked(competition, club) == 0


def test_loadPlacesAlreadyBooked_empty_list_should_return_zero(
        competition_with_empty_club_list, club_one):
    competition = competition_with_empty_club_list
    club = club_one
    assert loadPlacesAlreadyBooked(competition, club) == 0


def test_loadPlacesAlreadyBooked_other_clubs_participating_should_return_zero(
        competition_with_other_club, club_one):
    competition = competition_with_other_club
    club = club_one
    assert loadPlacesAlreadyBooked(competition, club) == 0


def test_loadPlacesAlreadyBooked_clubs_participating_should_return_four(
        competition_with_club_one, club_one):
    competition = competition_with_club_one
    club = club_one
    assert loadPlacesAlreadyBooked(competition, club) == 4


def test_updatePlaceBookedOrCreate_without_club_list(
        competition_without_club_list, club_one):
    competition = competition_without_club_list
    club = club_one
...
def test_updatePlaceBookedOrCreate_empty_club_list(
        competition_with_empty_club_list, club_one):
    competition = competition_with_empty_club_list
    club = club_one
    test = updatePlacesBookedOrCreate(competition, club, places_bought)
...
def test_updatePlaceBookedOrCreate_other_club(
        competition_with_other_club, club_one):
    competition = competition_with_other_club
    club = club_one
    test = updatePlacesBookedOrCreate(competition, club, places_bought)
...
def test_updatePlaceBookedOrCreate_with_club_one(
        competition_with_club_one, club_one):
    competition = competition_with_club_one
    club = club_one
    test = updatePlacesBookedOrCreate(competition, club, places_bought)
...
def test_purchasePlace_booking_impossible(client):
    ...
    data = rv.data.decode()
    assert rv.status_code == 200
    assert data.find('<p>You don&#39;t have enough points '
...
def test_purchasePlace_book_more_than_12_in_2_booking(client):
    resetDatabase(club, competition)
    places_bought = 7
    club_two = getClub(club)
    rv1 = client.post('/purchasePlaces', data=dict(
        club=club, competition=competition, places=places_bought))
    rv = client.post('/purchasePlaces', data=dict(
        club=club, competition=competition, places=places_bought))
    club_two = getClub(club)
    ...
    resetDatabase(club, competition)
    assert rv.status_code == 200
    assert data.find('<p>You can&#39;t book more than '
